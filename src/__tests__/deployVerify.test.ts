...
test('deployVerify validates upload by size from non-truncated listing', async () => {
	...
	await verifyUploadedFile(fs, '/home/root/lms2012/prjs/demo.rbf', bytes, DeployVerifyMode.SIZE);
});
...
test('deployVerify fails on size mismatch from listing snapshot', async () => {
	...
	await assert.rejects(
		verifyUploadedFile(fs, '/home/root/lms2012/prjs/demo.rbf', bytes, DeployVerifyMode.SIZE),
		/size mismatch/i
	);
});
...
test('deployVerify validates upload by md5 from non-truncated listing', async () => {
	...
	await verifyUploadedFile(fs, '/home/root/lms2012/prjs/demo.rbf', bytes, DeployVerifyMode.MD5);
});
...
test('deployVerify fails on md5 mismatch after readback fallback', async () => {
	...
	await assert.rejects(verifyUploadedFile(fs, '/home/root/lms2012/prjs/demo.rbf', local, DeployVerifyMode.MD5), /md5 mismatch/i);
});
