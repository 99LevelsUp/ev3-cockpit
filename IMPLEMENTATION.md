...
- `F7`: castecne hotovo:
  ...
  - pridana command akce `EV3 Cockpit: Apply Deploy Profile` (presety `Safe Sync`, `Atomic Sync`, `Full Sync`) pro rychle prepnuti relevantnich deploy/fs nastaveni.
...
## Context Compact Status (2026-02-08, pre-compact update)

Aktualni stav repozitare:
- branch: `master`
- remote: `origin/master` (`https://github.com/99LevelsUp/ev3-cockpit.git`)
- lokalni git stav v okamziku zapisu: 1 lokalni zmena (`IMPLEMENTATION.md`, tento status zapis)

Posledni klicove dodane zmeny:
- rozdeleni `extension.ts` command logiky do samostatnych modulu:
  - `src/commands/connectCommands.ts`
  - `src/commands/deployCommands.ts`
  - `src/commands/browseCommands.ts`
  - `src/commands/transportCommands.ts`
- ponechany puvodni funkcni zamer (connect/deploy/browse/run/emergency) bez regresniho oslabení scheduler safety.
- odstraneny nepouzivany fixture soubor `fixtures/hw/empty.rbf` (runtime i testy pouzivaji vestaveny fixture v `src/hw/fixtures/emptyProgram.ts`).
- host test suite vylepsena o per-case report (`pass/fail`) a stabilnejsi "commands without hardware" scenar pres explicitni `transport.mode=mock`.
- `transportHealthReport` respektuje explicitne zvoleny transport mode:
  - `mock` => report se korektne preskoci s informacni hlaskou,
  - `usb|tcp|bluetooth` => probe jen pro zvoleny mode,
  - `auto` => probe vsech realnych modu.

Aktualni overeni kvality:
- `npm run lint` -> OK
- `npm run test:unit` -> OK (174/174)
- `npm run test:host` -> OK (8/8)
- HW testy nejsou soucasti kazdeho CI behu; zustavaji manualne/optional.

Znamy neblokujici runtime noise:
- pri `test:host` se stale muze objevit VS Code/Electron log:
  - `Error mutex already exists`
  - obcasne `An unknown error occurred. Please consult the log for more details.`
- pri soucasnem `Exit code 0` + zelenych host testech je to evidovano jako neblokujici infrastructure noise, ne funkcni chyba extension logiky.

Aktualni provozni fokus:
- primarne USB + TCP/Wi-Fi stabilita a workflow dodelani,
- Bluetooth podpora zustava implementacne udrzovana (kod + scenare), ale tvrde HW verifikace BT lze docasne preskakovat pri host/driver nestabilite.

## Status Snapshot (2026-02-09)

Aktualni stav repozitare:
- branch: `master`
- git working tree: clean (`## master...origin/master`)
...
Poznamky k behu:
- host test run skoncil `Exit code 0`,
- neblokujici runtime warning z Node (`DEP0190`) se vyskytl i pri zelene suite.

Nejblizsi konkretni krok:
- manualni HW overeni Bluetooth stability bez `SKIP` fallbacku (`test:hw` / `test:hw:matrix`) v prostredi se stabilnim BT host stackem (COM lock/reopen scenare).

## Progress Update (2026-02-09, multi-brick UX follow-up)

Hotovo:
- per-brick runtime session management je vytazen do `src/device/brickSessionManager.ts` (oddelena lifecycle + program session metadata),
- pridan local profile store `src/device/brickConnectionProfiles.ts` (persistovany per-brick connect profil v `workspaceState`),
- reconnect vybrane kostky umi pouzit jeji ulozeny transport profil (host/port/path),
- pridan diagnosticky command `EV3 Cockpit: Inspect Brick Sessions`,
- pri zmene relevantni transport konfigurace extension nabidne reconnect aktualne ready sessions,
- pridan batch command set:
  - `EV3 Cockpit: Reconnect Ready Bricks`,
  - `EV3 Cockpit: Deploy Workspace to Ready Bricks`,
- tree rooty zobrazuji runtime busy counter a metadata posledni operace (`last operation`) v tooltipu.

Hotove testy:
- unit:
  - `brickSessionManager.test.ts`,
  - `brickConnectionProfiles.test.ts`,
  - rozsirene `brickRegistry.test.ts` + `brickTreeProvider.test.ts`,
- host:
  - novy scenar `batch commands with multi-brick mock tcp`,
  - stale zeleny multi-brick selected-deploy scenar.

Aktualni verifikace:
- `npm run lint` -> OK
- `npm run test:unit` -> OK (`185/185`)
- `npm run test:host` -> OK (`10/10`)

## Progress Update (2026-02-09, multi-brick UX implementation pass 2)

Hotovo:
- batch commandy umi cilit podmnozinu ready kostek (`QuickPick` multi-select nebo explicitni `brickId[]` argument),
- batch reconnect/deploy maji cancellable progress UX s per-brick reportingem a souhrnem,
- tree rooty umi perzistentni pin/favorite stav v `workspaceState` + pin-aware razeni root uzlu,
- `Inspect Brick Sessions` exportuje JSON report do `artifacts/diagnostics/brick-sessions-report.json`,
- pri config-change `Reconnect all` se pred reconnectem refreshne ulozeny profil kostky z aktualni workspace konfigurace,
- host suite ma explicitni pokryti reconnect prompt vetvi (`Later` i `Reconnect all`).

Aktualni verifikace:
- `npm run compile` -> OK
- `npm run lint` -> OK
- `npm run test` -> OK (`188/188`)
- `npm run test:host` -> OK (`11/11`)

Poznamka:
- Bluetooth hardwarove testy zustavaji mimo tento implementacni pass (dle aktualniho scope).
